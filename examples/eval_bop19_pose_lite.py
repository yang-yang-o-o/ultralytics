...
"""Evaluation script for the BOP Challenge 2019/2020 (lite: MSSD+MSPD only)."""
...
# Sibling scripts live in the upstream toolkit, not necessarily next to this lite wrapper.
BOP_SCRIPTS = os.environ.get("BOP_SCRIPTS", "/root/bop_toolkit/scripts")
...
################################################################################
p = {
    # Lite: MSSD + MSPD only (skip VSD — faster, no depth renderer workers).
    "errors": [
        {
            "n_top": -1,
            "type": "mssd",
...
for result_filename in result_filenames:
    ...
    # Final score for the given dataset (lite: mean over computed error types).
    final_scores["bop19_average_recall"] = float(
        np.mean([average_recalls[e["type"]] for e in p["errors"]])
    )
